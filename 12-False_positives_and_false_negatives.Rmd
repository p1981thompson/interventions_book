...
We will present two common techniques for FDR procedure, Benjamini and Hochberg (BH), and Benjamini and Yekutieli (BY). The BH procedure can be summarized into four steps:
...
3. Next we need to calculate a BH critical value for each p-value. We use the formula, $(i/n)Q$, where $i$ is a particular p-value's rank; $n$ is the number of tests; and $Q$ is the false discovery rate (typically, this is specified as a proportion or percentage, but there is not conventional value, it is upto the researhcer to decide on an appropriate level of expected false positives)
