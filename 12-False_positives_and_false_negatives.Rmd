...
```{r BH_example, echo=F,message=F,warning=F,results="asis"}
P_val_tab<-data.frame(Variable=paste0("X",1:10),Rank=1:10,alpha=rep(0.05,10),p_value=c(0.002,0.004,0.008,0.012,0.023,0.041,0.054,0.091,0.12,0.2))
P_val_tab$BH<-(P_val_tab$Rank/10)*P_val_tab$alpha
P_val_tab %>%
  mutate(BH = cell_spec(BH, color = "black", 
                    background = c(rep("#00FF00",4),rep("#FFFFFF",6))),
         p_value = cell_spec(p_value, color = "black", 
...
$(i/n)(\frac{\alpha}{\sum_{i=1}^{k} ^1/_i})$
