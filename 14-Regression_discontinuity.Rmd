...
## Compute densities for residuals.
    ...
    res <- data.frame(x=max(data.lm$x)- d$y, y=d$x+mean(data.lm$y))
    res <- res[order(res$y), ]
    ## Get some data for normal lines as well
    xs <- seq(min(data.lm$resid), max(data.lm$resid),length=100)
    res <- data.frame(y=xs + tail(sort(fitted.values(mylm1)),1), x=max(data.lm$x) - 2*dnorm(xs, 0, sd(data.lm$resid)))
