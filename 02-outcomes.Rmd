...
list_of_packages<-c("tidyverse","MASS","ggpubr")
...
```{r correlation, echo=F,message=F,warning=F,fig.width=9,fig.height=9,fig.cacp="Correlation plots"}
...
for(i in R_cors)
{
assign(myC[which(R_cors==i)],plot_cors(i) %>% ggplot(aes(x=x,y=y))+geom_point(alpha=0.25)+geom_smooth(method = "lm", se = FALSE, color = "red") + theme_bw() + xlab("") + ylab(""))
}
