...
{
cor_data<-as.data.frame(MASS::mvrnorm(40,c(3,3),Sigma=matrix(c(1,r,r,1),2,2)))
colnames(cor_data)<-c("x","y")
return(cor_data)
}
R_cors<-c(1,0.8,0.25,0)

myC<-paste0("C",1:4)
for(i in R_cors)
{
assign(myC[which(R_cors==i)],plot_cors(i) %>% ggplot(aes(x=x,y=y))+geom_point(alpha=0.75)+geom_smooth(method = "lm", se = FALSE, color = "red") + theme_bw() + xlab("Time 1 Score") + ylab("Time 2 Score")+ggtitle(paste0("r = ",R_cors[which(R_cors==i)])))
}
...
DB: I would omit this here; it's true we don't want to encourage people to overinterpret correlations, but I think it's giving them too much too soon. In effect, we'd just be getting across the idea of what a correlation is, and then saying you can't trust it! It is partially dealt with in the next section that shows how correlations may depend on age. We could cover Anscombe's quartet more fully later, perhaps when talking about visualising data; but here I think it could just be a distraction. -->
...
alldata$age<-c(rep(1,myn),rep(2,myn),rep(3,myn))
allcor <- round(cor(alldata$X1,alldata$X2),2)
mycolours<-c('red','blue','purple')

p <- ggplot(alldata,aes(x=X1,y=X2,color=mycolours[age]))+
  geom_point()+
   scale_fill_discrete(name = "Age (yr)", labels = c("3", "4", "5"))+
       theme_bw() +
       xlab("Time 1 Score") +
       ylab("Time 2 Score") +
       ggtitle(paste0("Overall r = ",allcor))
print(p)    
